...
const MainContentDashboard = () => {
  ...
  //   const { left, top, width, height, borderRadius } =
  //     activeButton.getBoundingClientRect();

  //   const { left: paginationLeft, top: paginationTop } =
  ...
  //   isPending,
  //   error,
  // } = useFetchData(`/generals/getTicketMgt/${userdata?.partnerId}`);
  // const openTickets = ticketData?.data?.filter(
  //   (ticket) => ticket.status === "open"
  // );

  // const pendingTickets = ticketData?.data?.filter(
  //   (ticket) => ticket.status === "pending"
  // );
  // const resolvedTickets = ticketData?.data?.filter(
  ...
  //                     ) : (
  //                       <p
  //                         className="card-text display-6 m-0"
  ...
  //               Partner Locations({partnerLoc?.length || 0})
  //             </h5>
